...
namespace Deform
{
	[Deformer (Name = "Wave2", Description = "Adds wave to mesh", XRotation = -90f, Type = typeof (WaveController2))]
    //[HelpURL("https://github.com/keenanwoodall/Deform/wiki/RippleDeformer")]
    public class WaveController2 : Deformer, IFactor
	{
		public float Factor
		{
			get => Amplitude;
		...
		public struct LimitedRippleJob : IJobParallelFor
		{
			...
			public void Execute (int index)
			{
               ...
               // var positionOffset = -1f * (point.x / d );
				...
				if (range != 0f)
				{
					...
					/*if(d > frequency - 0.1f && d < frequency + 0.1f){ 
						point.x += (frequency - d) * (1/(1f + 10f * (0.1f -  Mathf.Abs(frequency - d))));
						point.y += (frequency - d) * (1/(1f + 10f * (0.1f -  Mathf.Abs(frequency - d))));
					}*/

					if(d > frequency - 0.1f && d < frequency + 0.1f){ 
						point.x +=  ((frequency - d) / Mathf.Abs(frequency - d)) * amplitude;
						point.y += ((frequency - d) / Mathf.Abs(frequency - d)) * amplitude;
					}
