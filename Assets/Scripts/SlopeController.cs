...
namespace Deform
{
	[Deformer (Name = "Slope", Description = "Adds wave to mesh", XRotation = -90f, Type = typeof (SlopeController))]
    //[HelpURL("https://github.com/keenanwoodall/Deform/wiki/RippleDeformer")]
    public class SlopeController : Deformer, IFactor
	{
		public float Factor
		{
			get => Amplitude;
		...
		public struct LimitedRippleJob : IJobParallelFor
		{
			...
			public void Execute (int index)
			{
				...
				//var positionOffset = clampedD - amplitude  ;

				var positionOffset = (d - amplitude) * frequency  ;
